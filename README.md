...
- **입력**: Win32 `WM_POINTER`(`windows` 0.62) — **디지타이저(펜)만 필기**하게 하고
  ...
  압력도 없기 때문이다.
- **창**: 네이티브 WinUI `TitleBar`(캡션 버튼·드래그 영역·Mica가 전부 시스템 것) + Mica 배경,
...
└ Grid (가속기 Ctrl+±/Ctrl+Enter)
  ...
  │           + 잉크 미리보기(색·굵기) + **Diagnostics** 버튼
  ├ 정보 띠   상태 문구 · 힌트 · 도구/획/파이프라인/배율            parts/status.rs
  ├ 개발자 도구  디지타이저 진단 표 + 리스캔(호스트가 연다)          parts/devtools.rs
...
- `ui_plan` — 의도 21개 → 버튼 매핑(툴바/레일/복구/개발자 도구가 정확히 한 번씩), 상태 분기(빈/준비/여는
  중/실패), 상태 띠, **툴바 두 배치(한 줄·두 줄)가 기준 폭 안에 서는가**, **진단 패널이 정보 띠와
  본문 사이에 서는가**, `<Raw>` 경계가 **정확히 하나**이고 등록된 빌더에 재료가 그대로 도착하는가
- `digitizer` — 진단 표가 **세 관문**(펜 하드웨어 · 훅이 걸린 창 · 도착한 펜 메시지)을 정확히 말하는가,
  터치만 있는 기계를 펜이 있다고 하지 않는가, 화면 언어가 영어뿐인가
- `files` — 끌어놓기 규칙: **첫 PDF 하나**만 열고, 확장자는 대소문자를 가리지 않으며,
  PDF가 아니면 아무것도 열지 않는다
...
툴바의 **Diagnostics** 버튼이 진단 표를 연다(정보 띠와 본문 사이). 펜 입력은 **세 관문**을 다
지나야 성립하고, 표가 그 셋을 순서대로 말한다:
...
표는 다음 발행에서 갱신된다(포인터를 움직이면 바로 바뀐다). 필기가 되기 시작하면(`pen`이 늘면)
`digitizer::install()`은 창 찾기를 **그만둔다** — 확실해진 뒤에는 매 프레임 열거하지 않는다.
