...
//!
//! ## 왜 `pdf-writer`인가
//! hayro의 PDF 쓰기 크레이트(`hayro-write`)가 쓰는 writer가 `pdf-writer`다.
//! 그래서 **읽기(hayro)·쓰기(pdf-writer)** 가 같은 PDF 모델을 공유한다 —
//! 내보낸 PDF를 곧바로 hayro로 다시 읽어 검증할 수 있다(테스트가 그렇게 한다).
//!
//! ## 무엇을 쓰나
//! - 잉크는 **벡터 경로**로 쓴다(`m`/`l` + `S`) — 확대해도 계단이 없다.
//! - 필압으로 굵기가 변하면 구간별 폭으로 나눠 쓴다(래스터라이저와 같은 규칙).
...
impl std::fmt::Display for ExportError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ExportError::Raster(message) => write!(formatter, "이미지를 만들지 못했습니다: {message}"),
...
/// 페이지 하나를 PNG 바이트로 만든다(테스트/표면 계약이 그대로 쓴다).
pub fn page_to_png(
    page: &Page,
    background: Option<&Pixmap>,
    scale: f32,
) -> Result<Vec<u8>, ExportError> {
    let pixmap = raster::render_page_on_white(page, background, scale);
    raster::to_png(pixmap).map_err(ExportError::Raster)
}
...
///
/// 파일 이름은 `stem` + 페이지 번호다 — 같은 폴더에 여러 문서를 내보내도 섞이지 않는다.
pub fn export_document_png(
    ...
    let mut written = Vec::with_capacity(document.page_count());
...
pub fn document_to_pdf(
    ...
    // ① 반투명 스트로크(형광펜)용 ExtGState — 알파별로 하나.
    let alphas: Vec<u8> = document
        .pages()
        .iter()
    ...
    for page_refs in &pages {
        ...
        let rgb = raster::flatten_to_rgb8(&pixmap);
    ...
    // ⑥ 페이지 사전 + 리소스.
    for page_refs in &pages {
        let Some(page) = document.page(page_refs.index) else {
            continue;
        };
        let size = page.size();
...
pub fn export_pdf(
    ...
    let path = path.as_ref();
    let bytes = document_to_pdf(document, pdf)?;
    std::fs::write(path, bytes).map_err(|error| ExportError::Io(error.to_string()))?;
    Ok(path.to_path_buf())
}
