...
//! - 페이지 크기는 pt이며, 좌표는 모델과 같이 **좌상단 원점**으로 돌려준다.
    ...
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            PdfError::PageOutOfRange(index) => {
                write!(formatter, "페이지 {}가 없습니다", index + 1)
            }
        }
    }
}
...
impl PdfDocument {
    ...
    /// 모든 페이지 크기 — [`crate::Document::from_pdf_pages`]에 그대로 넘긴다.
    ...
    ///
    /// `scale` = 1pt당 픽셀 수. 배경은 **흰 종이**로 렌더한다(투명 배경 위에 필기하면
    /// PDF 뷰어마다 결과가 달라지기 때문).
    ...
    /// 페이지 배경을 `None`이면 건너뛴다(테스트/선택적 렌더 편의).
