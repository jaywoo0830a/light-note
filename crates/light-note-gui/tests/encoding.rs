...
//! | 파일 | 인코딩 | 줄바꿈 | 이유 |
//! |---|---|---|---|
//! | PowerShell(`*.ps1/.psm1/.psd1/.ps1xml`) | **UTF-8 with BOM** | CRLF | 5.1은 BOM이 없으면 ANSI로 읽는다 |
//! | `.bat`/`.cmd` | **ASCII만** | CRLF | `cmd.exe`는 OEM 코드페이지 — 한글은 `.ps1`로 |
//! | 그 외 텍스트 | UTF-8 **without** BOM | **CRLF**(윈도우) | rustc/cargo/깃은 항상 UTF-8 — BOM은 노이즈 |
//!
//! `.editorconfig`가 에디터의 **저장**을, `.gitattributes`가 **줄바꿈**을, `rustfmt.toml`이
//! rustfmt의 출력을 맡고, 여기서는 **결과**를 검사한다. 새 파일이 LF로 저장되거나 BOM 없이
//! 저장되는 드리프트를 이 테스트가 잡는다.
//!
//! ## 한계 (정직하게)
...
fn is_text_file(path: &Path) -> bool {
    ...
    // `Cargo.lock`은 **cargo가 소유**한다 — 화면·포맷과 무관하게 `\n`으로 다시 쓰이므로
    // 우리 CRLF 규칙의 대상이 아니다(규칙을 걸면 cargo가 파일을 갱신할 때마다 빨개진다).
    if name == "Cargo.lock" {
        return false;
    }
    matches!(
...
/// 검사 대상 텍스트 파일 전부(`.git`·`target`은 건너뛴다 — 우리 소스가 아니다).
...
fn collect(dir: &Path, files: &mut Vec<PathBuf>) {
    ...
    for entry in entries.flatten() {
        ...
        if path.is_dir() {
            // `.git`(저장소 메타데이터)·`target`(빌드 산출물)·`.port`(옛 크레이트 사본)은 제외.
...
fn the_walk_actually_finds_the_repository() {
    ...
    for anchor in [
        ...
        "rustfmt.toml",
        "run-windows.ps1",
        "crates/light-note-gui/src/lib.rs",
        "crates/light-note-gui/tests/encoding.rs",
    ] {
...
/// `.gitattributes`, 그리고 `rustfmt.toml`(`newline_style = "Windows"`)이 그렇게 맞추기로
/// 했고, 새로 만든 파일이 LF로 저장되는 드리프트를 여기서 잡는다(실제로 이 테스트가 그런
/// 파일들을 잡아냈다).
#[cfg(windows)]
#[test]
